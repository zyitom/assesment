...
std::vector<double> processingTimes; // Global to access in both threads
std::deque<FrameResult> frameQueue;  // Global to access in both threads
...
int main() {
    ...
    std::thread display_thread([&]() {
        while (true) {
            ...
            conditionVar.wait(lk, [&] { return exitDisplayThread || !frameQueue.empty(); });

            if (exitDisplayThread && frameQueue.empty()) {
                break;
            }
...
std::cout << "处理第 " << sequenceNumber << " 帧" << std::endl; // 添加这一行来监控正在处理的帧序号
...

        {
            ...
            conditionVar.wait(lock, [&] { return frameQueue.size() < maxQueueSize; });
        }
    }
